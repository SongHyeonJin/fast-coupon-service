...
```
[Client] ─── 쿠폰 발급 요청 ───▶ [Spring Boot API]
                                        │
                                        ▼
                                  ┌─────────────┐
                                  │  Redis Queue │  ← LPUSH (요청 적재)
                                  └──────┬──────┘
                                         │
                                         ▼
                                ┌─────────────────┐
                                │  Redis Worker    │  ← BLPOP (순차 소비)
                                │  + Lua Script    │  ← 중복 체크 + 재고 확인 + 발급 (원자적)
                                │  + 분산 락(SETNX) │  ← 멀티 서버 워커 중복 방지
                                └────────┬────────┘
                                         │ 발급 성공
                                         ▼
                                  ┌─────────────┐
                                  │   Kafka      │  ← 이벤트 발행 (coupon.issue)
                                  └──────┬──────┘
                                         │
                              ┌──────────┴──────────┐
                              ▼                     ▼
